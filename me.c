...
static void sad_block_8x8(const uint8_t* const orig, const uint8_t* const ref,
		int stride, __m128i* const result)
{
...
static void sad_block_2x8x8(const uint8_t* const orig, const uint8_t* const ref,
		int stride, __m128i* const result1, __m128i* const result2)
{
	__m128i ref_pixels = _mm_loadu_si128((void const*)(ref));
...
static void get_sad_index(const __m128i min_values, const __m128i min_indexes,
		int* sad_index_x, int* sad_index_y) {
...
static void me_block_2x8x8(struct c63_common *cm, int mb1_x, int mb_y,
	...
	const __m128i row_incrementor = _mm_set1_epi16(range_width / 8 + 1);
	...
	__m128i sad_min_indexes_block1 = _mm_setzero_si128();
	__m128i sad_min_values_block2 = m128i_epi16_max;
	__m128i sad_min_indexes_block2 = _mm_setzero_si128();
	...
	for (y = top; y < bottom; ++y) {
		...
		if (mb1_left < mb2_left) {
			...
			__m128i cmpgt = _mm_cmpgt_epi16(sad_min_values_block1,
					next_min_block1);
			sad_min_values_block1 = _mm_min_epi16(sad_min_values_block1,
					next_min_block1);
			sad_min_indexes_block1 = _mm_blendv_epi8(sad_min_indexes_block1,
					counter, cmpgt);
		}
		...
		for (x = mb2_left; x < mb1_right; x += 8) {
			...
			sad_block_2x8x8(orig_pointer, ref_pointer, w, &next_min_block1,
					&next_min_block2);
			...
			sad_min_values_block2 = _mm_min_epi16(sad_min_values_block2,
					next_min_block2);
			sad_min_indexes_block2 = _mm_blendv_epi8(sad_min_indexes_block2,
					counter, cmpgt);
		...
		if (mb2_right > mb1_right) {
			...
			__m128i cmpgt = _mm_cmpgt_epi16(sad_min_values_block2,
					next_min_block2);
			sad_min_values_block2 = _mm_min_epi16(sad_min_values_block2,
					next_min_block2);
			sad_min_indexes_block2 = _mm_blendv_epi8(sad_min_indexes_block2,
	...
	get_sad_index(sad_min_values_block1, sad_min_indexes_block1, &sad_index_x,
			&sad_index_y);
	mb1->mv_x = normalized_left + sad_index_x - m1x;
	mb1->mv_y = top + sad_index_y - my;
	mb1->use_mv = 1;

	get_sad_index(sad_min_values_block2, sad_min_indexes_block2, &sad_index_x,
			&sad_index_y);
...
static void mc_block_8x8(struct c63_common *cm, int mb_x, int mb_y,
	...
	struct macroblock *mb = &cm->curframe->mbs[color_component][mb_y
			* cm->padw[color_component] / 8 + mb_x];
